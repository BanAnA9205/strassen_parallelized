...
constexpr int maxParallelDepth = 3;
...
void _strassenOMP(const MatrixView A, const MatrixView B, MatrixView C, size_t n, int depth) {
    if (n < threshold) { mul(A, B, C, n, n, n); return; }
    ...
    size_t half_n_sq = half_n * half_n;
    auto mem{new float[half_n_sq * 11]};
    MatrixView S1{mem, half_n};
    MatrixView S2{mem + half_n_sq, half_n};
    MatrixView S3{mem + half_n_sq * 2, half_n};
    MatrixView S4{mem + half_n_sq * 3, half_n};
    MatrixView T1{mem + half_n_sq * 4, half_n};
    MatrixView T2{mem + half_n_sq * 5, half_n};
    MatrixView T3{mem + half_n_sq * 6, half_n};
    MatrixView T4{mem + half_n_sq * 7, half_n};
    MatrixView P1{mem + half_n_sq * 8, half_n};
    MatrixView U2{mem + half_n_sq * 9, half_n};
    MatrixView U3{mem + half_n_sq * 10, half_n};
    ...
    #pragma omp task
    _strassenOMP(A11, B11, P1, half_n, depth);
    #pragma omp task
    _strassenOMP(A12, B21, C11, half_n, depth);
    #pragma omp task
    {
        sub(A12, S2, S4, half_n);
        _strassenOMP(S4, B22, C12, half_n, depth);
    }
    #pragma omp task
    {
        sub(T2, B21, T4, half_n);
        _strassenOMP(A22, T4, C21, half_n, depth);
    }
    #pragma omp task
    _strassenOMP(S1, T1, C22, half_n, depth);
    #pragma omp task
    _strassenOMP(S2, T2, U2, half_n, depth);
    #pragma omp task
    {
        sub(A11, A21, S3, half_n);
        sub(B22, B12, T3, half_n);
        _strassenOMP(S3, T3, U3, half_n, depth);
    }
    #pragma omp taskwait
...
void strassenMPI(const MatrixView A, const MatrixView B, MatrixView C, size_t n) {
    ...
    if (size < 8) {
        if (rank == 0) {
            std::cout << "Not enough MPI processes for parallel Strassen, running _strassenOMP on rank 0." << std::endl;
            _strassenOMP(A, B, C, n, 0);
        }
        return;
    }
    ...
    if (rank == 0) {
        ...
        {
            #pragma omp single
            _strassenOMP(AA, BB, CC, half_n, 0);
        }
...
float maxDiff(const float *c1, const float *c2, size_t n) {
    float max_diff = 0.0f;
...
int main(int argc, char** argv) {
    ...
    if (rank == 0) {
        ...
        std::cout << "Max Parallel Depth: " << maxParallelDepth << std::endl;
        if (argc < 2) {
    ...
    if (rank == 0) {
        std::cout << "StrassenOMPI: " << (t1 - t0) << " seconds." << std::endl;
    }
