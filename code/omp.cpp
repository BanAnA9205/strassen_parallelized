...
void strassenOMP(const MatrixView a, const MatrixView b, MatrixView c, size_t n) {
    #pragma omp parallel
    {
        #pragma omp single
...
int main(int argc, char** argv) {
    ...
    strassenOMP({aMem.data(), size}, {bMem.data(), size}, {cMem.data(), size}, size);
    auto t1 = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> elapsed = t1 - t0;
    std::cout << "StrassenOMP: " << elapsed.count() << " seconds." << std::endl;
}